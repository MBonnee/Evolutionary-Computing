...
public class Algorithm {
	...
	public ArrayList<Individual> reproduction(ArrayList<Individual> parents, double splitPerc, int crossOver,
			double mutationRate, double mutationSize) {
	...
	public Individual nPointCrossOver(Individual indiv1, Individual indiv2, int num) {
		...
		case 4:
			...
			break;
		default:
			randomPointCrossOver(indiv1, indiv2, places, child);
			break;
		}
		return child;
	}

	public void randomPointCrossOver(Individual indiv1, Individual indiv2, LinkedList<Integer> places,
			Individual child) {
	...
	private void threePointCrossOver(Individual indiv1, Individual indiv2, LinkedList<Integer> places,
			Individual child) {
		for (int i = 0; i < child.size(); i++) {
	...
	private void fourPointCrossOver(Individual indiv1, Individual indiv2, LinkedList<Integer> places,
			Individual child) {
		for (int i = 0; i < child.size(); i++) {
	...
	public Individual mutateRandom(Individual indiv, double mutationProbability, double mutSize) {
		...
		for (int i = 0; i < indiv.size(); i++) {
			if (Math.random() < mutationProb) {
				if(mutSize != 5) {
					double add = Math.random() * 2 * mutSize - mutSize;
					double gen = mutated.getGene(i);
					if(add + gen > 5) {
						indiv.setGene(i, add - gen);
					}else if(add + gen < -5) {
						indiv.setGene(i, add - gen);
					}else {
						indiv.setGene(i, add + gen);
					...
					mutated.setGene(i, (Math.random()* 10 - 5));
				}
