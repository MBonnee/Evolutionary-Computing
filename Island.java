...
public class Island {
	...
	private double islandMutationSize;
	...
	public HashMap<Integer, Integer> getDiversity(int numIslands) {
		...
		for (Individual ind : population.individuals) {
			if (frequencymap.containsKey(ind.initialIsland)) {
				...
				frequencymap.put(ind.initialIsland, 1);
			}
		}
	...
	private void updateEvolveRate() {
		...
		if (popFitness < 4.0) {
			...
			islandSplitPerc = 0.3;
			islandCrossOverNr = 0;
			islandMutationSize = 3;
		} else {
			islandMutationRate = 0.2;
			islandSplitPerc = 0.6;
			islandCrossOverNr = 0;
			islandMutationSize = 1;
		}
	...
	public void evolvePopulation(ArrayList<Individual> parents) {
		...
		ArrayList<Individual> children = alg.reproduction(parents, islandSplitPerc, islandCrossOverNr,
				islandMutationRate, islandMutationSize);
